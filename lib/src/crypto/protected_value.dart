...
class NFCValue implements StringValue {
  NFCValue(this._value, this._salt);

  factory NFCValue.fromString(String value) {
    final valueBytes = utf8.encode(value) as Uint8List;
    final salt = _randomBytes(valueBytes.length);

    return NFCValue(_xor(valueBytes, salt), salt);
  }

  factory NFCValue.fromBinary(Uint8List value) {
    final salt = _randomBytes(value.length);
    return NFCValue(_xor(value, salt), salt);
  }
  ...
  @override
  bool operator ==(dynamic other) =>
      other is NFCValue && other.getText() == getText();
  ...
  @override
  String toString() {
    return 'NFCValue{${base64.encode(hash)}}';
  }
}
